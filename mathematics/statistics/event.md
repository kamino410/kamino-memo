...
偶然現象を試行したとき起こりうる結果すべての集合を**標本空間**または**全事象**といい、$$\Omega$$ で表す。

>例

>サイコロをふるとき $$\Omega = \{1,2,3,4,5,6\}$$ が標本空間である
...
>サイコロをふるとき $$1,2,3,4,5,6$$ が標本点である

### 事象（event）

標本空間の部分集合を**事象**という。
また、ただ1つの標本点からなる事象を**根源事象（elementary event）**といい $$A,B,\cdots$$ で表す。
標本点を1つも含まない事象を**空事象**といい $$\emptyset$$ で表す。

>例

>サイコロをふるとき $$\{1,2\}$$ は「1か2が出る」という事象を意味し、空事象は「生じ得ない」事象を意味する
...
>サイコロをふるとき $$\{1,2\} \cup \{2,3\} = \{1,2,3\}$$ は「1か2か3が出る」という事象を示す
...
>サイコロをふるとき $$\{1,2\} \cap \{3,4\} = \emptyset$$ は「1か2が出て同時に3か4が出る」というありえない事象を意味するため、$$\{1,2\}$$ と $$\{3,4\}$$ は排反である
...
>直観的には条件1は全事象を含むすなわち確率1を定義できること、条件2はある事象が起きる確率に対して起きない確率を定義できること、条件3はある事象とある事象が同時に生じる確率を定義できることを示している。
