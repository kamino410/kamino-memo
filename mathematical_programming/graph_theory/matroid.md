...
と定義される関数を**閉包関数**という。
つまり、ある部分集合に追加しても階数が変わらないような要素の集合を示す関数である。
...
## 基本サーキット（fundamental circuit）

閉包関数の性質2で示した従属集合 $$I \cup \{j\}$$ はただ1つのサーキットを含む（サーキットの性質3より）。
このサーキットを**基本サーキット**といい、$$C(I|j)$$ と表す。
すなわち

$$
C(I|j) = \{ i \ | \ (I \cup \{j\}) \backslash \{i\} \in \mathcal{I}\}\ (I \in \mathcal{I}, \ j \in \mathrm{cl}(I)\backslash I)
$$

である。

## 双対マトロイド（dual matroid）

マトロイド $$\mathbf{M} = (E,\mathcal{I})$$ の基族 $$\mathcal{B}$$ に対して $$\mathcal{B}^* = \{ E \backslash B \ | \ B \in \mathcal{B} \}$$ を定義すると、$$\mathcal{B}^*$$ も基族の性質を満たす。
これを基族とするマトロイド $$\mathbf{M}^* = (E,\bigcup_{B^*=\mathcal{B}^*}^n 2^{B^*})$$ を**双対マトロイド**という。

元のマトロイドの階数関数と双対マトロイドの階数関数の関係は次のように表される。

$$
\rho^*(X) = |X| + \rho(E\backslash X) - \rho(E)
$$

双対マトロイドにおけるサーキット $$\mathcal{C}^*$$ を $$\mathbf{M}$$ の**コサーキット（cocircuit）**という。
任意のサーキット $$\mathcal{C}$$ と任意のコサーキット $$\mathcal{C}^*$$ に対して $$|\mathcal{C} \cap \mathcal{C}^*| \neq 1$$ となる。

また、定義から明らかなようにコサーキットはすべての基と空でない交わりを持っている。
任意の基 $$B \in \mathcal{B}$$ とその要素 $$b \in B$$ に対して、$$\mathcal{C}^* \cap B = \{b\}$$ となるコサーキットが一意に存在する。
このコサーキットを $$B$$ と $$b$$ に関する**基本コサーキット**といい、$$\mathcal{C}^*(B|b)$$ と表す。
すなわち、

$$
C^*(B|b) = \{e \ | \ (B\backslash \{b\}) \cup \{e\} \in \mathcal{B} \} \ (b \in B \in \mathcal{B})
$$
...
* 基本サーキットの例
  ...
  * $$C(\{1,2\}|3)=\{1,2,3\}$$
  * $$C(\{1,2\}|4)=\{4\}$$
* 双対マトロイドの基族：$$\mathcal{B}^* = \{\ \{1,4\},\{2,4\},\{3,4\}\ \}$$
* コサーキット：$$\mathcal{C}^* = \{\ \{1,2\},\{1,3\},\{2,3\}\ \}$$
* 基本コサーキットの例
  * $$C^*(\{1,2,3\}|3) = \{3\}$$
  * $$C^*(\{4\}|4) = \{4\}$$
...
* 基本サーキットの例
  ...
  * $$C^*(\{1,3\}|2) = \{1,2\}$$
  * $$C^*(\{1,3\}|4) = \{3,4\}$$
* 双対マトロイドの基族：$$\mathcal{B}^* = \{\ \{1,2\},\{3,4\}\ \}$$
* コサーキット：$$\mathcal{C}^* = \{\ \{1,3\},\{1,4\},\{2,3\},\{2,4\}\ \}$$
* 基本コサーキットの例
  * $$C^*(\{1,3\}|1) = \{1,2\}$$
  * $$C^*(\{1,3\}|3) = \{3,4\}$$
  * $$C^*(\{2,3\}|2) = \{1,2\}$$
